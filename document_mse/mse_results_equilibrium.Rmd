...
``` 
Now, zooming in on the last few years to better see the seasonal dynamics. Note that the low point of the annual cycle is actually the last day of the year before the labeled year, just before recruitment occurs.  So, we can see the decline in biomass happening during the year due to natural mortality.
```{r, fig.cap="Coastwide SSB by season, zoomed in to last 3 years of the projection"} 
 ggplot(tot_ssb_dat, aes(time,tot)) +
   ...
   xlim(2115,2118)
```
If we plot ssb by country seasonally, and compare against the area specific ssb_0 (horizontal black lines), we start to see the effects of the movement matrix.  Neither country stabilizes at the ssb_0 values specified in the area-specific stock-recruit curves.  
...
```
Like before, let's zoom in to see the equilibrium dynamics better. The annual mortality pattern seen in the previous graph is apparent in the US portion of the population, but not in Canada because fish are moving into the northern area. 

Something that isn't making sense to me here is that ssb in Canada is still high on the last day of the year, which should be after 85 percent of the biomass has moved south.  Similarly, ssb in the US is at the lowest point on the last day of the year. 
...
When we zoom in and look at the last 4 years, we see the same pattern as with seasonal ssb.  Note that vulnerable biomass (v_save) is saved each quarter before movement occurs but after natural mortality is applied. Vulnerable biomass peaks in the US in season 1 and declines through season 4.  In Canada, biomass is highest in seasons 3 and 4, and drops in season 1.  
